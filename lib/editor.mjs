 ...
 *     （行数 + 字节 + 单行字符数三重预算）；备份文件名等细节只保留在台账与 `stdout` 中。完整 diff 经
 ...
 *   maxDiffBytes: number          模型可见 diff 正文的字节预算，默认 4096
 *   maxDiffLineChars: number      模型可见 diff 单行的字符上限，默认 200
...
const EDIT_DESCRIPTION =
  ...
  + 'content up, and returns a stat line plus the changed lines when the diff is small (see `diff`). '
  ...
  + 'wrong place. An ambiguous match is refused unless `nth` or `count` disambiguates it. Set `dry_run` to '
  + 'preview without writing.'

const WRITE_DESCRIPTION =
  'Create or completely replace one text file. Preserves the UTF-8 BOM and the file line-ending style, backs '
  + 'the previous content up, and returns a stat line instead of echoing the content back (see `diff`). '
  + 'A missing target is created, missing parent directories included; a brand-new file follows the '
  + 'line-ending style of its sibling files. Set `dry_run` to preview without writing.'
...
export function apply(ctx, config) {
  ...
  const planOptions = {
    ...
    ...(Number.isFinite(settings.maxDiffLines) && settings.maxDiffLines >= 1
      ...
      : {}),
    ...(Number.isFinite(settings.maxDiffBytes) && settings.maxDiffBytes >= 1
      ? { maxDiffBytes: Math.floor(settings.maxDiffBytes) }
      : {}),
    ...(Number.isFinite(settings.maxDiffLineChars) && settings.maxDiffLineChars >= 1
      ? { maxDiffLineChars: Math.floor(settings.maxDiffLineChars) }
      : {}),
  }
