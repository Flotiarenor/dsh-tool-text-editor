 ...
 * （`lib/core.mjs`）补上，并把 `grep` / `lines` 锚点与忽略空白的匹配一起带上：
...
const EDIT_DESCRIPTION =
  ...
  + '(`replace@17 +1/-1`) instead of echoing the change. `after`/`before` need `grep`/`lines`. '
  + 'Give exactly ONE anchor: `old_text` (replace only), `grep` (regex) or `lines`. Prefer '
  + '`old_text`/`grep` for replace: a wrong line number edits the wrong place without failing. '
  + 'A literal matching more than once is refused unless `count` declares how many you expect. '
  + 'Whitespace is ignored while matching (spaces, blank lines, line breaks); a character '
  + 'difference is refused, naming the first line that differs.'
...
export const EDIT_PARAMETERS = {
  ...
  properties: {
    ...
    mode: { type: 'string', description: 'replace (default) substitutes the anchor; after/before insert beside a grep/lines anchor only (never old_text); append/prepend use the file ends.', enum: EDIT_MODES },
