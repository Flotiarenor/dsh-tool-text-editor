 ...
 *   * **忽略空白的匹配**：精确 → 去掉空白后相同（空格/缩进/空行/换行一律忽略）→ 失败时给**一处**差异；
 *     命中不确定时拒绝写盘而不是猜（原生只做精确匹配）；
...
// ─────────────────────────────────────────────────────────────────────────────
//
// 判定只有一条：**去掉空白后相同**。空白 = 空格 / tab / CR / LF / FF / VT 这六个 ASCII 字符，出现在哪里
// 都算（行首缩进、行尾空格、整行空行、换行符本身）。锚点的职责是**定位**，不是复述格式：抄来的锚点少写
// 行尾空格、多写一个空行、缩进不同，都该命中同一处，而不是让调用方去猜自己哪里写错了。
//
// 刻意不用 `\s`：它还匹配 U+3000 全角空格 / NBSP / BOM。中文文件里全角空格是**内容**，当成空白会让
// "锚点写半角空格、文件里是全角空格"这种真实的字符差异被静默写盘。
const BLANK = /[ \t\r\n\f\v]/
...
/**
 * 命中区间是否贴住整行：第一个非空白字符必须是它所在行的第一个非空白字符，最后一个非空白字符必须是它
 * 所在行的最后一个非空白字符。
 *
 * 换行也被忽略之后，锚点可能只匹配到某一行的**一部分**——`const x = 1;` 在 `const x = 1;const y = 2;`
 * 里就是子串——而按行跨度替换会把那一行剩下的内容一起吃掉。要求首尾贴住整行之后，缩进 / 空行 / 行尾
 * 空白仍然全忽略，被挡住的只有"形状不同"这一件事。
 */
function atLineEdges(text, start, end) {
  for (let i = start - 1; i >= 0 && text[i] !== '\n'; i -= 1) if (!BLANK.test(text[i])) return false
  for (let i = end; i < text.length && text[i] !== '\n'; i += 1) if (!BLANK.test(text[i])) return false
  return true
}

/** 去空白后的第 from..to 个字符（闭区间）落在原文的哪一段：`[start, end)`。 */
function mapBack(text, from, to) {
  let seen = -1
  let start = -1
  for (let i = 0; i < text.length; i += 1) {
    if (BLANK.test(text[i])) continue
    seen += 1
    if (seen === from) start = i
    if (seen === to) return [start, i + 1]
  }
  return [-1, -1]
}
...
/**
 * 忽略空白的全部命中（原文坐标）。返回**全部**命中而不只最好的那一个：命中多处时匹配层要拒写，
 * 所以这里必须能把"不止一处"如实报出来。
 * @returns `[{ start, end, edges }]`；`edges === false` 表示"去空白能对上，但只盖住半行"。
 */
function blankMatches(text, needle) {
  const hay = stripBlank(text)
  const found = []
  let from = 0
  while (true) {
    const index = hay.indexOf(needle, from)
    if (index < 0) break
    const [start, end] = mapBack(text, index, index + needle.length - 1)
    if (start < 0) break
    found.push({ start, end, edges: atLineEdges(text, start, end) })
    from = index + 1
  }
  return found
}
...
/**
 * 把命中区间扩成整行块。`[start, end)` 的首尾都是非空白字符，所以只需往外找到行边界。
 *
 * 锚点以换行结尾 = 调用方在按行边界寻址 → 连行尾换行符一起换掉；否则换行符留给文件，不然替换会把下一行
 * 并上来（`+1/-2` 是模型唯一的线索）。
 */
function wholeLines(text, start, end, endsWithNl) {
  let from = start
  while (from > 0 && text[from - 1] !== '\n') from -= 1
  let to = end
  while (to < text.length && text[to] !== '\n') to += 1
  if (to < text.length && endsWithNl) to += 1
  return [from, to]
}

/** 提示里能放的一行原文：截到 100 字符再转义，行内的引号 / 制表符撑不破报文。 */
function quoteLine(line) {
  return JSON.stringify(line.length > 100 ? `${line.slice(0, 100)}…` : line)
}

/** 两行第一处不同的字符位置（1-based）；没有不同时为 0。 */
function firstCharDifference(a, b) {
  const n = Math.min(a.length, b.length)
  let i = 0
  while (i < n && a[i] === b[i]) i += 1
  return i < n || a.length !== b.length ? i + 1 : 0
}

/**
 * 失败时的**唯一**一处提示：从"锚点第一处非空行"在文件里的对应行开始，一行一行比到第一处不同。
 *
 * 逐行比而不是逐字符比：文件某行尾部多了几个字符时，逐字符比会在跳过锚点的换行符之后把指针推到**下一行**，
 * 报出"你的第 4 行 vs 文件第 3 行"这种错位的对比——而调用方要看的是同一对行。
 *
 * 不像以前的滑动窗口那样列几段互相重叠的候选、也不报相似度：报一处、给出两侧原文，能直接照着改。
 * @returns 一行提示（不含缩进）；锚点没有任何非空行时返回 `''`。
 */
function firstDifference(text, old) {
  const oldLines = splitLines(old).lines
  const fileLines = splitLines(text).lines
  const anchorLine = oldLines.findIndex((line) => stripBlank(line) !== '')
  if (anchorLine < 0) return ''
  const key = stripBlank(oldLines[anchorLine])
  const fileLine = fileLines.findIndex((line) => stripBlank(line) === key)
  if (fileLine < 0) {
    return `your old line ${anchorLine + 1} occurs nowhere in the file: ${quoteLine(oldLines[anchorLine])}`
  }
  let a = anchorLine
  let b = fileLine
  while (a < oldLines.length) {
    if (stripBlank(oldLines[a]) === '') { a += 1; continue }
    while (b < fileLines.length && stripBlank(fileLines[b]) === '') b += 1
    if (b >= fileLines.length) break
    if (stripBlank(oldLines[a]) !== stripBlank(fileLines[b])) {
      const at = firstCharDifference(oldLines[a], fileLines[b])
      return `first difference: your old line ${a + 1} ${quoteLine(oldLines[a])}`
        + ` vs file line ${b + 1} ${quoteLine(fileLines[b])}${at > 0 ? ` (char ${at})` : ''}`
    }
    a += 1
    b += 1
  }
  if (a >= oldLines.length) {
    return b < fileLines.length
      ? `the file has more lines after your old: file line ${b + 1} ${quoteLine(fileLines[b])}`
      : ''
  }
  return `the file ends before your old does: your old line ${a + 1} ${quoteLine(oldLines[a])}`
}

/** 未命中的统一形状：`note` 说原因，`hint` 说下一步该怎么做（至多一行）。 */
function miss(note, hint = '') {
  return { ok: false, spans: [], mode: 'miss', note, hint }
}

/**
 * 在 text 里定位 old：精确 → 忽略空白（见本节开头）→ 失败时给**一处**差异提示。
 *
 * 只有一个消歧旋钮：`expect`（声明的命中数）。它一旦声明就只走精确路径——`count` 的语义是"我知道它精确
 * 出现几次"，不是"放宽到忽略空白之后出现几次"。没有"只取第 k 次出现"这类入口：那是**选择**而非**确认**，
 * 与"命中多处即拒绝、由调用方把锚点写准"的契约相反（此前的 `nth` 正因绕过它，才让错锚点变成静默的错编辑）。
 *
 * @param text - 逻辑文本。
 * @param old - 旧片段。
 * @param options - `expect`（要求恰好 N 处；不符即拒绝）。
 * @returns { ok, spans, mode, note, hint }
 */
export function matchLiteral(text, old, options = {}) {
  const { expect } = options
  // 空锚点必须**先**判：`findAllExact` 的 `from = index + max(1, 0)` 会在原地打转并撑爆内存。
  if (old === '') return miss('old must not be empty')
  const hits = findAllExact(text, old)
  if (expect !== undefined && hits.length === expect) {
    return { ok: true, spans: hits, mode: expect === 1 ? 'exact' : `exact:count(${expect})`, note: expect === 1 ? '' : `${expect} hits, all replaced`, hint: '' }
  }
  if (hits.length === 1) return { ok: true, spans: hits, mode: 'exact', note: '', hint: '' }
  if (hits.length > 1) {
    const starts = lineStarts(text)
    ...
    return { ok: false, spans: [], mode: 'ambiguous', note: reason, hint: '' }
  }
  if (expect !== undefined) {
    return miss(`old occurs exactly 0 times, expected ${expect}`,
      'count only counts exact occurrences: drop count to allow a whitespace-insensitive match, or fix the anchor')
  }
  const needle = stripBlank(old)
  if (needle === '') {
    return miss('old is nothing but whitespace, so it cannot locate anything — use a lines or grep anchor')
  }
  const found = blankMatches(text, needle)
  const onLines = found.filter((entry) => entry.edges)
  // 去空白能对上、却只盖住半行：形状不同，替换会连带吃掉那半行剩下的内容，一律拒写。
  if (onLines.length === 0 && found.length > 0) {
    const starts = lineStarts(text)
    const line = offsetToLine(starts, found[0].start)
    return miss(
      `old matches at line ${line} once whitespace is ignored, but it covers only part of that line — refusing to write`,
      `Quote the whole line, or use a lines / grep anchor. File line ${line}: ${quoteLine(splitLines(text).lines[line - 1])}`,
    )
  }
  if (onLines.length > 1) {
    // 忽略空白后命中多于一处：与精确命中同样拒绝写盘，绝不按文件顺序悄悄挑一处。
    const starts = lineStarts(text)
    const where = onLines.slice(0, 8).map((entry) => offsetToLine(starts, entry.start)).join(', ')
    return {
      ok: false,
      spans: [],
      mode: 'ambiguous',
      note: `old does not match exactly; ignoring whitespace it hits ${onLines.length} places (lines ${where}) —`
        + ' quote a longer old to narrow it down, or use a lines / grep anchor (a whitespace-insensitive hit is never a choice: there is no way to know it picked the same place)',
      hint: '',
    }
  }
  if (onLines.length === 0) {
    return miss('old does not exist in the target file (exact and whitespace-insensitive matching both failed)', firstDifference(text, old))
  }
  // 命中区间落在整行上：按行跨度替换，文件自己的空行与缩进（跨度之外的）原样留下。
  return {
    ok: true,
    spans: [wholeLines(text, onLines[0].start, onLines[0].end, old.endsWith('\n'))],
    mode: 'ws',
    note: 'exact match failed; matched ignoring whitespace (spaces, blank lines, line breaks) — the gaps were filled from the file; check that the change landed where you meant',
    hint: '',
  }
}
    ...
    if (kind === 'write') {
      ...
      if (plan.mode === 'append' || plan.mode === 'prepend') {
        ...
        if (plan.mode === 'replace') {
          ...
          if (!match.ok) {
            // 全角冒号是中文文案的遗留：模型可见文本自 i18n 起一律英文。
            return fail(plan.filePath, `${label}: ${match.note}${match.hint ? `\n  ${match.hint}` : ''}`)
          }
          // 锚点路径的 mode 带 `anchor:` 前缀；去掉它之后，"是不是精确命中"才可判。
          const hitMode = String(match.mode).replace(/^anchor:/, '')
          // 这里不逐处比对 `plan.count`：`count` 的校验各有其主——`grep` / `lines` 由 `resolveAnchor`
          // 在命中处校验，`old_text` 由 `matchLiteral` 的 `expect` 校验。锚点路径的 span 是"锚点文本在
          // 全文出现几次"，逐处替换时天然为一处，拿它比 `count` 只会把正当的多处替换误判成失败。
          ...
          // 只通报忽略空白的命中：`exact` 与 `exact:count(N)` 带的是"命中 N 处，全部替换"这类说明，不是警告。
