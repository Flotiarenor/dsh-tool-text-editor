 ...
 *     那是另一件事，README 里写明了）。
 *
 * 模型可见文本只有 `brief`：一行统计（`replace@17 +1/-1`）加必要的警告。**不回显改动内容**——
 * 工具结果按追加方式进入会话历史，任何内容回显都会随调用次数累积，而调用方刚发过 `new_text`，
 * 需要看正文时 `read` 一次即可。改动记录由落盘前的备份（`<工作区>/.dsh/backups/`）与编辑台账
 * （`<工作区>/.dsh/edits.log` 的 JSONL，字段见 `appendLedger`）承担，二者都不进入模型上下文。
 */
import {
  closeSync,
  existsSync,
 ...
 *
 * plan（edit）：`{ kind:'edit', filePath, mode, newText, oldText, anchor:{value}|null, count, nth }`
      ...
      if (plan.mode === 'append' || plan.mode === 'prepend') {
        ...
        if (plan.mode === 'replace') {
          ...
          if (plan.anchor) {
            ...
            match = matchLiteral(original, target, { nth: plan.nth ?? 0 })
